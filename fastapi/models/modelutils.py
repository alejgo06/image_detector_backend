...
def get_precisionDF(outputs, targets):
    # https://medium.com/analytics-vidhya/map-mean-average-precision-for-object-detection-with-simple-python-demonstration-dcc7b3850a07
    outputs = [{k: v.to('cpu') for k, v in t.items()} for t in outputs]
    targets = [{k: v.to('cpu') for k, v in t.items()} for t in targets]
    df = pd.DataFrame({})
    for i in range(len(targets)):
        if outputs[i]['boxes'].shape[0]!=0:
            pred_coor = outputs[i]['boxes'][0]  # [outputs[i]['scores']==min(outputs[i]['scores'])][0]
            real_coor = targets[i]['boxes'][0]
            img_name = targets[i]['image_id']

            xmin_pred, ymin_pred, xmax_pred, ymax_pred = pred_coor
            xmin, ymin, xmax, ymax = real_coor
            df = df.append({'image_name': str(int(img_name[0])),
                            'xmin': int(xmin),
                            'xmin_pred': int(xmin_pred),
                            'ymin': int(ymin),
                            'ymin_pred': int(ymin_pred),
                            'xmax': int(xmax),
                            'xmax_pred': int(xmax_pred),
                            'ymax': int(ymax),
                            'ymax_pred': int(ymax_pred)
                            }, ignore_index=True)
    if df.shape[0]==0:
        precisionDF= pd.DataFrame({})
    else:
        precisionDF = precision_recall(df)
    return precisionDF
